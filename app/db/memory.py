...
async def session_add(
    redis: aioredis.Redis, config: Config, session_id: str, role: str, content: str
) -> None:
...
async def db_migrate(config: Config) -> None:
    ...
    async with pool.acquire() as conn:
        ...
        await conn.execute(f"""
            ...
            WITH (list = {config.ivfflat_lists})
        """)
        await conn.execute(
...
async def ltm_search(config: Config, topic: str) -> dict | None:
    ...
    async with pool.acquire() as conn:
        row = await conn.fetchrow(
            """
            SELECT id, topic, report, created_at, 1 - (embedding <=> $1::vector) AS similarity
            FROM reports
